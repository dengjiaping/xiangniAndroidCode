...
public final class DPCManager {
    ...
    private boolean useDateCache = true;
    /**
     * 是否使用日期的缓存
     * @param flag
     */
    public void setUseInfoCache(boolean flag){
        useDateCache = flag;
    }

    private void setDecor(List<String> date, HashMap<String, Set<String>> cache) {
        //转换成datepicker能使用的日期格式2017-08-03->2017-8-3
        List<String> formatDate = new ArrayList<>();
        for(String str1:date){
            formatDate.add(DataUtils.toAigeDate(str1));
        }
        date=formatDate;
        for (String str : date) {
            ...
            String day = str.substring(index + 1, str.length());
            days.add(day);
            cache.put(key, days);
        }
    }
