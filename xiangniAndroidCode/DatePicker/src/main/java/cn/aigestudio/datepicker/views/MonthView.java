...
public class MonthView extends View {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                } else if (mSlideMode == SlideMode.VER && verticalScrollEnable) {
            ...
            case MotionEvent.ACTION_UP:
                if (mSlideMode == SlideMode.VER) {
                    if (Math.abs(lastPointY - event.getY()) > 25 && verticalScrollEnable) {
    ...
    private void draw(Canvas canvas, int x, int y, int year, int month) {
        ...
        for (int i = 0; i < result.length; i++) {
            for (int j = 0; j < result[i].length; j++) {
                ...
                String strG = dpInfo.strG;
                draw(canvas, tmp[i][j].getBounds(), dpInfo);
            }
        }
        canvas.restore();
    }
    ...
    private void drawGregorian(Canvas canvas, Rect rect, String str, boolean isWeekend) {
        ...
        //选中颜色
        String curDate = centerYear + "-" + centerMonth + "-" + str;

        if (dateSelected.contains(curDate) || cirApr.containsKey(curDate)) {
            Log.i(TAG, "drawGregorian: " + curDate);
    ...
    private BGCircle createCircle(float x, float y, String date) {
        OvalShape circle = new OvalShape();
        circle.resize(0, 0);
        ShapeDrawable drawable = new ShapeDrawable(circle);
        BGCircle circle1 = new BGCircle(drawable);
        ...
        drawable.getPaint().setColor(circleBG == null ? mTManager.colorBGCircle() : circleBG.getCircleBg(date));
        return circle1;
    }

    public CircleBG circleBG;

    public CircleBG getCircleBG() {
        return circleBG;
    }

    public void setCircleBG(CircleBG circleBG) {
        this.circleBG = circleBG;
    }

    public interface CircleBG {
        int getCircleBg(String date);
    }
            ...
            for (int j = 0; j < tmp[i].length; j++) {
                ...
                if (region.contains(x, y)) {
                    ...
                    if (mDPMode == DPMode.SINGLE) {
                        ...
                        if(interceptor!=null&&!interceptor.isDateCanSelected(date)){
                            return;
                        }
                        ...
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB && showAnim) {
                        ...
                        if (regions.contains(region)) {
                            ...
                            if (onDateClick == null || onDateClick.isDateCanSelect(date)) {
                                regions.add(region);
                                dateSelected.add(date);
                                if (onDateClick != null) {
                                    onDateClick.beforeSelect(date);
                                }
                                BGCircle circle;
                                if (circleBG == null) {
                                    circle = createCircle(
                                    ...
                                    circle = createCircle(
                                            region.getBounds().centerX() + indexMonth * width,
                                            region.getBounds().centerY() + indexYear * height, date);
                                }
    ...
    public interface DateInterceptor{
        boolean isDateCanSelected(String date);
    }

    private DateInterceptor interceptor;

    public DateInterceptor getInterceptor() {
        return interceptor;
    }
    ...
    public interface OnDateClick {
        ...
        //点击了一个选中的日期，将要取消
        void beforeCancel(String date);

        //点击了一个未选中的日期，将要选中,返回值判断是否能被选中
        boolean beforeSelect(String date);

        boolean isDateCanSelect(String date);
    }

    private OnDateClick onDateClick;

    public OnDateClick getOnDateClick() {
        return onDateClick;
    }
