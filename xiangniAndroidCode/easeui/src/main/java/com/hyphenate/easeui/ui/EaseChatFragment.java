...
public class EaseChatFragment extends EaseBaseFragment implements EMMessageListener {
    ...
    private final int REQUEST_PERMISSON_CAPTURE = 17;
...
//                if(event.getAction()==MotionEvent.ACTION_DOWN){
                    if(ActivityCompat.checkSelfPermission(getContext(),Manifest.permission.RECORD_AUDIO)==PackageManager.PERMISSION_DENIED){

                        Toast.makeText(getContext(),"您已拒绝了录音权限，无法使用此功能,请到设置中修改权限.",Toast.LENGTH_LONG).show();
                        return false;
                    }
//                }
            ...
            public void onBigExpressionClicked(EaseEmojicon emojicon) {
                if(emojicon instanceof IXNEmotionIcon){
                    sendIXNExpressionMessage((IXNEmotionIcon)emojicon);
                }else {
    ...
    protected void setUpView() {
        ...
        if (chatType == EaseConstant.CHATTYPE_SINGLE) {
            ...
            titleBar.setRightImageResource(R.drawable.zhanghaoxinxi);
        } else {
        	titleBar.setRightImageResource(R.drawable.qun);
    ...
    private void sendIXNExpressionMessage(IXNEmotionIcon emojicon) {
        File iconFile = emojicon.getIconFile();
        if(iconFile.exists()){
            sendImageMessage(emojicon.getIconFile().getAbsolutePath());
        }
    ...
    protected void selectPicFromCamera() {
        ...
        int result = ActivityCompat.checkSelfPermission(getContext(), Manifest.permission.CAMERA);
        if(result== PackageManager.PERMISSION_GRANTED){//通过权限
            takeCapture();
        }else {
//            requestPermissions(new String[]{Manifest.permission.CAMERA},101);

            Toast.makeText(getContext(),"您已拒绝了相机权限，无法使用此功能,请到设置中修改权限.",Toast.LENGTH_LONG).show();
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        if(requestCode==101){
            if(grantResults[0]==PackageManager.PERMISSION_GRANTED){
                takeCapture();
                Toast.makeText(getContext(), "权限通过", Toast.LENGTH_SHORT).show();
            }else {
                Toast.makeText(getContext(), "您禁止了相机权限，\n无法使用拍照功能...", Toast.LENGTH_SHORT).show();
            }
        }
    }
